...
@bot.command()
async def stonks(ctx, username, input):
    ...
    if sign == "+":
        if amount_positive - integer >= 0:
            ...
            await ctx.send("Not enough daily currency! Positive amount remaining: " + str(amount_positive) + ". Negative amount remaining: " + str(amount_negative))
            return
    elif sign == "-":
